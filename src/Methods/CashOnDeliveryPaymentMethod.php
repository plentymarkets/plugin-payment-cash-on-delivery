...
class CashOnDeliveryPaymentMethod extends PaymentMethodBaseService
{
    ...
    public function isActive(): bool
    {
        ...
        $list = $this->parcelServicePresetRepoContract->getLastWeightedPresetCombinations(
            $this->basketRepo->load(),
            $contactClassId,
            $params
        );

        foreach ($list as $id => $parcelService) {
            $parcelPreset = $this->parcelServicePresetRepoContract->getPresetById(
                $parcelService['parcelServicePresetId']
            );
